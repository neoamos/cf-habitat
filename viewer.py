...
class ImgThread(threading.Thread):
    def __init__(self, deck_ip, deck_port, callback=None, viewer=True):
    ...
    def run(self):
        print("Connecting to socket on {}:{}...".format(self.deck_ip, self.deck_port))
            ...
            if start_idx > -1 and end_idx > -1 and end_idx > start_idx:
                ...
                if self._callback:
                    ...
                    try:
                        ...
                        self._callback(img, imgdata)
                    except Exception:
                        print("Error parsing image")

                if self.viewer:
                    try:
                        self.viewer._showframe(imgdata)
...
class FrameViewer(Gtk.Window):
    ...
    def __init__(self):
        ...
        self.img_counter = 0
        self.last_img_saved_at= time.time()
    ...
    def _showframe(self, imgdata):
        ...
        try:
            ...
            # if img_path and pix != None: #and (time.time() - self.last_img_saved_at) > 1:
            #     self.img_counter += 1
            #     self.last_img_saved_at = time.time()
            #     pix.savev(img_path + "/" + str(self.img_counter) + ".jpg", 'jpeg', ["quality"], ["100"])
...
if __name__ == "__main__":
  ...
  img_thread = ImgThread(deck_ip, deck_port)
  img_thread.start()
  Gtk.main()
